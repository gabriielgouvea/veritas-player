# dashboard.py (Versão 11.0 - Full Integration)
...
# Importante: O downloader agora é um Frame, não mais uma janela separada
...
class DashboardWindow(ctk.CTkToplevel):
    def __init__(self, parent, player):
        ...
        ctk.CTkLabel(self.sidebar, text="v11.0 - Integrado", text_color="#AAA", font=("Segoe UI", 10, "bold")).pack(side="bottom", pady=(0, 10))
    ...
    def show_view(self, view_name):
        ...
        if view_name == "config": self.render_config()
            ...
            # Agora instanciamos o Frame do Downloader dentro da main_area
            # Passamos self.main_area como pai e a pasta de treino atual
            YoutubeDownloader(self.main_area, self.player.pasta_treino).pack(fill="both", expand=True)
    ...
    # --- CRIAR/EDITAR (COM FIX ID) ---
    def render_ad_create(self):
        d = {}
        if self.editando_id:
            # Busca segura por String
        ...
        fd = ctk.CTkFrame(t1, fg_color="transparent"); fd.pack(fill="x", pady=10)
        ...
        fs = ctk.CTkFrame(t1, fg_color="transparent"); fs.pack(fill="x")
        self.d_vars = {}; self.chk_dias = []
        ...
        fdt = ctk.CTkFrame(t2, fg_color="transparent"); fdt.pack(fill="x")
